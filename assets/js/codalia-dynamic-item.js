...
Codalia.DynamicItem = class {
  constructor(props) {
    // Sets the item properties.
    this.vendor = props.vendor;
    this.pluginName = props.plugin;
  ...
  */
  setItemOrdering(idNb) {
    ...
    attribs = {'class':'ordering-div', 'id':this.itemType+'-ordering-div-'+idNb};
    document.getElementById(this.itemType+'-row-'+row+'-cell-'+cell+'-'+idNb).appendChild(this.createElement('div', attribs));
    ...
    document.getElementById(this.itemType+'-ordering-div-'+idNb).appendChild(this.createElement('input', attribs));
    ...
    attribs = {'src':this.rootLocation+'/plugins/'+this.vendor+'/'+this.pluginName+'/assets/images/arrow_down.png', 'title':'arrow down', 'height':16, 'width':16};
    link.appendChild(this.createElement('img', attribs));
    document.getElementById(this.itemType+'-ordering-div-'+idNb).appendChild(link);
    ...
    attribs = {'src':this.rootLocation+'/plugins/'+this.vendor+'/'+this.pluginName+'/assets/images/arrow_up.png', 'title':'arrow up', 'height':16, 'width':16};
    link.appendChild(this.createElement('img', attribs));
    document.getElementById(this.itemType+'-ordering-div-'+idNb).appendChild(link);
  ...
  */
  validateFields(fields, extraType) {
	...
	let Select2 = null;
	if(this.Select2 && (field.type == 'select-one' || field.type == 'select-multiple')) {
	...
	if(field.value == '') {
	  ...
	  if(this.nbItemsPerPage !== null) {
	    ...
	    this.updatePagination(pageNb);
          ...
          alert(CodaliaLang.message.alert_mandatory_field);
	  return false;
	}
  ...
  */
   ...
   * @return  void
  */
  createDateTimeFields(name, idNb, rowCellId, value, time) {
    ...
    attribs = {'type':'text', 'id':this.itemType+'-date-'+name+'-'+idNb, 'class':'form-control', 'autocomplete':'off', 'data-datepicker':''};
    ...
    attribs = {'type':'hidden', 'name':this.itemType+'_'+name+'_'+idNb, 'id':'publication-'+name+'-'+idNb, 'value':value, 'data-datetime-value':''};
